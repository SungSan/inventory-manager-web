-- SAN WMS V4.6.7
-- Dashboard real inbound/outbound filter
--
-- 집계 원칙
--   * 일반 입고/출고 포함
--   * 외부이관 포함 (실제 창고 밖 출고)
--   * 업무요청 포함 (실제 창고 밖 출고)
--   * 단순 재고이관(reference_type='TRANSFER')만 제외
--   * 단순 재고이관을 취소한 REVERSAL 거래도 제외
--   * 한국시간 00:00~06:59 거래는 모든 집계에서 제외
--   * 일간 시간대 표시는 07:00~23:00만 생성
--
-- SQL 41을 실행하지 않은 DB에서도 단독 실행할 수 있습니다.
...
begin
  ...
  where t.created_at>=v_start_ts
    ...
    and coalesce(t.reference_type,'')<>'TRANSFER'
    and (t.created_at at time zone 'Asia/Seoul')::time >= time '07:00:00'
    and not exists (
      select 1
      from public.inventory_transactions original
      where original.id=t.reversal_of
        and coalesce(original.reference_type,'')='TRANSFER'
    );

  if v_period='DAY' then
    with buckets as (
      select generate_series(
        v_start_date::timestamp+interval '7 hours',
  ...
  return jsonb_build_object(
    ...
    'series',v_series,
    'aggregation_mode','REAL_FLOW_EXCLUDE_TRANSFER_0700',
    'aggregation_start_hour',7,
    'generated_at',now()
  );
end;
$$;
...
select 'SAN WMS V4.6.7 dashboard real flow filter migration completed' as result;
