-- SAN WMS V4.5.9
-- 업무요청 + 외부이관 출고명세서 통합 레지스트리 / 공통 문서번호
--
-- 원칙
--   * 기존 EXT-* / WR-SHIP-* 문서번호와 원본 문서는 그대로 보존한다.
--   * 신규 문서는 출처와 무관하게 OUT-YYYYMMDD-NNNN 공통 번호를 사용한다.
--   * 외부이관/업무요청의 기존 재고 차감 및 완료 RPC는 수정하지 않는다.
--   * 원본 명세서 테이블은 유지하고 공통 레지스트리에서 통합 조회한다.
--   * 업무요청 강제 완료의 요청/실제/미출고 정보도 통합 문서에서 보존한다.

begin;

-- SQL 35 적용 여부와 무관하게 통합 조회에 필요한 스냅샷 컬럼을 보장한다.
...
-- 레거시 문서는 혹시 과거 수동 중복 번호가 있더라도 보존한다.
-- V4.5.9 이후 OUT 번호만 전역 유일성을 DB에서 강제한다.
...
begin
  if tg_table_name='external_shipment_documents' then
    insert into public.shipment_document_registry(
      ...
      new.created_at
    ) on conflict(source_type,source_document_id) do nothing;
...
-- 기존 완료 RPC가 생성하려는 EXT-/WR-SHIP- 번호를 INSERT 직전에 OUT 번호로 교체한다.
...
-- 구버전 외부이관 명세서 화면에서 담당자를 수정해도 통합 레지스트리와 동기화한다.
...
select 'SAN WMS V4.5.9 unified shipment documents migration completed' as result;
