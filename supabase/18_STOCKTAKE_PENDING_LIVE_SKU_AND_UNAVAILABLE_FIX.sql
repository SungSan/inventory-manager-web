...
-- 2) PENDING LOC의 전산 SKU를 현재 재고 기준으로 반환
-- 3) 실제 빈 LOC 여부를 is_currently_empty로 명시 반환
...
-- 기존 작업에 들어간 사용불가 LOC 중 아직 시작하지 않은 항목을 제거한다.
...
begin
  ...
  select jsonb_build_object(
    ...
    'locations', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'system_sku_count', case
            when sl.status = 'PENDING' then stock.current_sku_count
            else sl.system_sku_count
          end,
          'is_currently_empty', case
            when sl.status = 'PENDING' then stock.current_sku_count = 0
            else false
          end,
      ...
      left join lateral (
        select count(*)::integer as current_sku_count
...
select 'SAN WMS V3.8.3 pending live SKU and unavailable LOC fix completed' as result;
