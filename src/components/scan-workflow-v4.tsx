...
export function ScanWorkflowV4() {
  ...
  function activateProductWorkflow(items: ProductDraft[], barcodeValue: string) {
    ...
    setFeedback({
      kind: "success",
      title: items.length > 1 ? "복수 상품 확인 완료" : "상품 확인 완료",
      body: `${items.length} SKU가 확인됐습니다. 입고 또는 출고를 선택하세요.`,
    });
  }
  ...
  function chooseOperation(nextOperation: MovementType) {
  ...
  async function confirmProductBatch() {
    if (!operation) {
      setFeedback({ kind: "error", title: "입고 또는 출고를 선택하세요." });
      return;
    }
