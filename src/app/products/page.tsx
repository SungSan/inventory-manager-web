...
function ProductsContent() {
  ...
  async function save() {
    ...
    } catch (cause) { setFeedback({ kind: "error", title: "상품 저장 실패", body: cause instanceof Error ? cause.message : "오류" }); }
    finally { setBusy(false); }
  }

  async function removeProduct(product: Product) {
    const label = `${product.artist || "아티스트 미등록"} · ${product.nameVer || product.codeNo}`;
    if (!window.confirm(`${label}\n\n이 상품을 완전히 삭제할까요?\n현재 재고·입출고 이력·재고이관 기록이 있으면 삭제되지 않습니다.`)) return;
    setDeletingId(product.id); setFeedback(null);
    try {
      await deleteUnusedProduct(product.id);
      if (editingId === product.id) { setEditingId(null); setForm(emptyForm); }
      setFeedback({ kind: "success", title: "상품 삭제 완료", body: label }); await load();
    } catch (cause) { setFeedback({ kind: "error", title: "상품 삭제 실패", body: cause instanceof Error ? cause.message : "상품을 삭제하지 못했습니다." }); }
  ...
  return <div className="page-stack">
    ...
    <section className="panel"><div className="section-heading"><div><h3>{editingId ? "상품 정보 수정" : "신규 상품 즉시 등록"}</h3></div>{editingId ? <button className="button button-secondary button-compact" onClick={() => { setEditingId(null); setForm(emptyForm); }}>수정 취소</button> : null}</div><div className="form-grid">
      ...
      <button className="button button-primary span-two" disabled={busy || !form.codeNo.trim() || !form.artist.trim() || !form.nameVer.trim() || (!editingId && !form.primaryBarcode.trim())} onClick={() => void save()}>{busy ? "저장 중..." : editingId ? "수정 저장" : returnTo ? "상품 등록 후 실사로 돌아가기" : "상품 등록"}</button>
    </div></section>
    {feedback ? <Feedback kind={feedback.kind} title={feedback.title}>{feedback.body}</Feedback> : null}
    <section className="panel"><div className="section-heading"><h3>등록 상품</h3><CameraSearchField label="검색" value={search} onChange={setSearch} placeholder="바코드, 코드, 아티스트, 상품명" /></div><div className="table-wrap"><table><thead><tr><th>상태</th><th>아티스트</th><th>상품명/버전</th><th>P_CODE</th><th>CODE_NO</th><th>MASTER</th><th>관리</th></tr></thead><tbody>{rows.map((product) => <tr key={product.id}><td><span className={`status-badge ${product.active ? "active" : "inactive"}`}>{product.active ? "사용" : "중지"}</span></td><td>{product.artist}</td><td>{product.nameVer}</td><td>{product.pCodeNo}</td><td>{product.codeNo}</td><td>{product.masterCodeNo}</td><td><div className="row-actions"><button className="button button-secondary button-compact" onClick={() => startEdit(product)}>수정</button><button className="button button-ghost button-compact" onClick={() => void updateProduct(product.id, { active: !product.active }).then(load)}>{product.active ? "비활성화" : "활성화"}</button>{canDelete ? <button className="button button-danger button-compact" disabled={deletingId === product.id} onClick={() => void removeProduct(product)}>{deletingId === product.id ? "삭제 중..." : "삭제"}</button> : null}</div></td></tr>)}</tbody></table></div>{rows.length === 0 ? <p className="empty-state">등록된 상품이 없습니다.</p> : null}</section>
  </div>;
}
