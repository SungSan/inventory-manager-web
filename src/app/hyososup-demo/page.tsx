...
export default function HyososupDemo(){
  ...
  function createRequest(e:React.FormEvent){e.preventDefault();if(!Number.isInteger(draft.qty)||draft.qty<1||!draft.needed){setMessage("요청 수량과 필요일을 정확히 입력하세요.");return}const manager=stores.find(x=>x.id===store)?.manager??"지점장";const order:Order={id:Math.max(...data.orders.map(x=>x.id))+1,store,item:draft.item,qty:draft.qty,needed:draft.needed,note:draft.note||"재고 보충 요청",status:"요청",by:manager};setData(x=>({...x,orders:[order,...x.orders]}));setRequestOpen(false);setView("orders");setMessage(`본사 담당자에게 ${storeName(store)}의 ${itemOf(draft.item).name} 요청을 알렸습니다.`)}
  ...
  function movement(kind:"in"|"use"){if(!move)return;if(!Number.isInteger(moveQty)||moveQty<1){setMessage("처리 수량은 1 이상의 정수로 입력하세요.");return}const current=data.stock[store]?.[move]??0;if(kind==="use"&&current<moveQty){setMessage("현재 재고보다 많이 사용할 수 없습니다.");return}setData(x=>({...x,stock:{...x.stock,[store]:{...x.stock[store],[move]:kind==="in"?current+moveQty:current-moveQty}}}));setMessage(`${storeName(store)} ${itemOf(move).name} ${kind==="in"?"입고":"사용"} ${moveQty}${itemOf(move).unit}를 반영했습니다.`);setMove(null);setMoveQty(1)}
