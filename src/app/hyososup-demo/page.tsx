...
+}
+
+function Title({eyebrow,title,body,action,actionLabel="+ 물품 요청"}:{eyebrow:string;title:string;body:string;action?:()=>void;actionLabel?:string}){return <header className={styles.title}><div><small className={styles.eyebrow}>{eyebrow}</small><h1>{title}</h1><p>{body}</p></div>{action?<button className={styles.primary} onClick={action}>{actionLabel}</button>:null}</header>}
