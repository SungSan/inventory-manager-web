...
function LocationMapContent() {
  ...
  async function setActive(location: Location, active: boolean) {
    ...
    try {
      ...
      if (!active && selectedId === location.id) setSelectedId("");
  ...
  return (
    <div className="page-stack location-map-page">
      <section>
        ...
        <h2>LOC MAP</h2>
        <p className="muted">
          로케이션 코드를 숫자 순서로 자동 배치합니다. 예를 들어 D1B-01-01-04는 01-01-03과 01-02-01 사이에 표시됩니다.
        </p>
      </section>
      ...
      <section className="metric-grid four">
        <article className="metric-card"><span>활성 LOC</span><strong>{activeLocations.length.toLocaleString()}</strong></article>
        <article className="metric-card"><span>점유 LOC</span><strong>{occupiedCount.toLocaleString()}</strong></article>
        <article className="metric-card"><span>빈 LOC</span><strong>{Math.max(0, activeLocations.length - occupiedCount).toLocaleString()}</strong></article>
        <article className="metric-card"><span>제외 LOC</span><strong>{inactiveLocations.length.toLocaleString()}</strong></article>
      </section>

      <section className="panel location-map-toolbar">
      ...
      <div className="location-map-workspace">
        <div className="location-map-zones">
          {zones.map((zone) => (
            <section key={zone.zone} className="panel location-zone-panel">
              ...
              <div className="location-zone-columns" style={{ gridTemplateColumns: `repeat(${Math.max(1, zone.columns.length)}, minmax(132px, 1fr))` }}>
        ...
        <aside className="panel location-detail-panel">
          {!selected ? <p className="empty-state">LOC를 선택하면 상세 재고가 표시됩니다.</p> : (
            <>
