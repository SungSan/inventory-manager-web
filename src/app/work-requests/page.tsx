...
function WorkRequestsContent() {
  ...
  const loadRequests=useCallback(async()=>{
    try{
      ...
      if(tab!=="new")setRequests(await listWorkRequests(scope,tab==="all"));
      setError("");
  ...
  const visibleTabs:Tab[]=["new","own",...(user?.role!=="viewer"?["work" as Tab]:[]),...(user?.role==="admin"||user?.role==="manager"?["all" as Tab]:[])];
  const tabLabel:Record<Tab,string>={new:"새 출고요청",own:"내 요청",work:"내 작업함",all:"전체 요청"};

  return <div className={`page-stack ${styles.page}`}>
    <section className="section-heading"><div><p className="eyebrow">WORK REQUESTS</p><h2>업무요청</h2><p className="muted">조회자는 외부이관 형식으로 출고를 요청하고, 배정 작업자가 실제 상품·LOC 바코드를 스캔한 수량만 재고에서 차감합니다. 출고명세서는 상단 통합 출고명세서 메뉴에서 조회합니다.</p></div>{user?.role==="admin"?<Link className="button button-secondary" href="/work-requests/admin">KPI·휴무일 설정</Link>:null}</section>
    ...
    {tab!=="new"?<section className="panel page-stack"><div className="section-heading"><div><p className="eyebrow">REQUEST LIST</p><h3>{tabLabel[tab]}</h3></div><button className="button button-secondary button-compact" onClick={()=>void loadRequests()}>새로고침</button></div>{loading?<p className="empty-state">업무요청을 불러오는 중입니다.</p>:null}<div className={styles.requestList}>{requests.map((request)=>{
