...
function playTone(kind: "SCAN" | "COMPLETE" | "ERROR") {
  try {
    ...
    const frequencies =
      kind === "COMPLETE" ? [660, 880] : kind === "ERROR" ? [180, 120] : [920];
    frequencies.forEach((frequency, index) => {
      const oscillator = context.createOscillator();
      const gain = context.createGain();
      oscillator.frequency.value = frequency;
      oscillator.type = kind === "ERROR" ? "square" : "sine";
      gain.gain.value = kind === "ERROR" ? 0.16 : 0.07;
      oscillator.connect(gain);
      gain.connect(context.destination);
      const start = context.currentTime + index * 0.1;
      oscillator.start(start);
      oscillator.stop(start + (kind === "ERROR" ? 0.13 : 0.08));
    });
  } catch {
    /* 소리가 차단되어도 스캔은 계속한다. */
  }
...
async function buildJob(
  ...
  for (const barcode of barcodes.filter((item) => item.active)) {
    const key = normalizeBarcode(barcode.value);
    const targets = barcodeTargets.get(key) ?? [];
    if (!targets.includes(barcode.targetId)) targets.push(barcode.targetId);
    barcodeTargets.set(key, targets);
  }
  const grouped = new Map<string, Map<string, OutboundUploadRow[]>>();
  for (const row of uploadRows) {
    const shipment =
      grouped.get(row.trackingNo) ?? new Map<string, OutboundUploadRow[]>();
    const key = normalizeBarcode(row.productBarcode);
    shipment.set(key, [...(shipment.get(key) ?? []), row]);
    grouped.set(row.trackingNo, shipment);
  }
  const shipments: OutboundShipment[] = [...grouped.entries()].map(
    ([trackingNo, itemGroups]) => {
      ...
      return {
        ...
        items,
        manualQuantityAllowed: false,
        status: items.some((item) => item.resolution !== "MATCHED")
          ? "REVIEW"
          : "READY",
      };
    },
...
function OutboundProgressContent() {
  ...
  function showFocus(
    item: OutboundPickingItem,
    kind: NonNullable<FocusState>["kind"],
    timeout = 0,
  ) {
  ...
  function updateShipment(
    shipmentId: string,
    updater: (shipment: OutboundShipment) => OutboundShipment,
  ) {
    setJobs((current) =>
      current.map((job) =>
  ...
  function startShipment(shipment: OutboundShipment) {
    if (shipment.status === "REVIEW") {
      playTone("ERROR");
      setFeedback({
        kind: "error",
        title: "확인 필요 운송장입니다.",
        body: "미등록 또는 공통 바코드를 먼저 정리하세요.",
      });
      return;
    }
    ...
    updateShipment(shipment.id, (current) => ({
      ...
      assignedWorker: user?.displayName,
    }));
    setFeedback({
      kind: "info",
  ...
  function scanItem(raw: string) {
    ...
    updateShipment(activeShipment.id, (shipment) => {
      ...
      return {
        ...
        items,
        status: items.every(
          (candidate) => candidate.pickedQty === candidate.requiredQty,
        )
          ? "COMPLETED"
          : "IN_PROGRESS",
      };
  ...
  function submitScan(event: React.FormEvent) {
    ...
    const value = scanner.trim();
    setScanner("");
    if (!value) return;
    if (!activeShipment) {
      const shipment = selectedJob?.shipments.find(
        (candidate) =>
          normalizeBarcode(candidate.trackingNo) === normalizeBarcode(value),
      );
      if (!shipment) {
        playTone("ERROR");
        setFeedback({
          kind: "error",
          title: "출고 작업에 없는 운송장입니다.",
          body: value,
        });
  ...
  function applyManualQuantity() {
    ...
    if (
      ...
      add < 1 ||
      focus.item.pickedQty + add > focus.item.requiredQty
    ) {
      playTone("ERROR");
  ...
  async function upload() {
    ...
    try {
      ...
      setFeedback({
        ...
        title: "출고 작업 생성 완료",
        body: `운송장 ${job.shipments.length}건 · ${rows.length}개 상품 행`,
      });
    } catch (cause) {
      setFeedback({
        kind: "error",
  ...
  const counts = useMemo(
    () =>
      selectedJob
        ? {
            total: selectedJob.shipments.length,
            done: selectedJob.shipments.filter(
              (item) => item.status === "COMPLETED",
            ).length,
            review: selectedJob.shipments.filter(
      ...
      {creating ? (
        <section className="panel">
          <div className="section-heading">
            <div>
              <h3>출고 데이터파일 2개 업로드</h3>
              <p className="muted">
                주문번호로 두 파일을 연결하고 자체품목코드(88바코드) 기준으로
                상품을 구성합니다.
              </p>
            </div>
            <button
          ...
          <div className="form-grid">
            <label>
              1. 운송장번호 파일 *
              <small className="muted">원송장 / 주문번호</small>
              <input
                type="file"
                accept=".xlsx,.xls"
                disabled={busy}
                onChange={(event) =>
                  setTrackingFile(event.target.files?.[0] ?? null)
                }
              />
            </label>
            <label>
              2. 상품수량 파일 *
              <small className="muted">주문번호 / 자체품목코드 / 수량</small>
              <input
                type="file"
                accept=".xlsx,.xls"
                disabled={busy}
                onChange={(event) =>
                  setQuantityFile(event.target.files?.[0] ?? null)
                }
              />
            </label>
            <button
              className="button button-primary span-two"
              disabled={busy || !trackingFile || !quantityFile}
              onClick={() => void upload()}
            >
      ...
      {jobs.length ? (
        <section className="panel">
          ...
          <div className={styles.shipmentList}>
            {jobs.map((job) => (
              <button
                className={`button ${selectedJobId === job.id ? "button-primary" : "button-secondary"}`}
                key={job.id}
                onClick={() => {
                  setSelectedJobId(job.id);
                  setActiveShipmentId("");
      ...
      {selectedJob && counts ? (
        <>
          <section className={styles.jobGrid}>
            ...
            <article className="metric-card">
              <span>진행·대기</span>
              <strong>{counts.total - counts.done - counts.review}</strong>
            </article>
            <article className="metric-card">
              <span>확인 필요</span>
          ...
          {activeShipment ? (
            <section className="panel">
              <div className="section-heading">
                <div>
                  <h3>필요 물품</h3>
                  <p className="muted">
                    품목을 스캔하면 집중 화면으로 전환됩니다. 다른 품목을 찍으면
                    해당 품목으로 즉시 전환됩니다.
                  </p>
                </div>
              </div>
              <div className={styles.itemList}>
                {activeShipment.items.map((item) => (
                  <article
                    className={`${styles.item} ${item.pickedQty === item.requiredQty ? styles.itemComplete : ""}`}
                    key={item.id}
                  >
                    <div>
                      <strong>
                        {item.artist} · {item.nameVer}
                      </strong>
                      <div className={styles.locations}>
                        {item.locations.length ? (
                          item.locations.map((location) => (
                            ...
                            >
                              {location.locationCode} · {location.qty}
                            </span>
                          ))
                        ) : (
                          <span className="inline-error">재고 LOC 없음</span>
                        )}
                      </div>
                    </div>
                    <div>
            ...
            <section className="panel">
              ...
              <div className={styles.shipmentList}>
                {selectedJob.shipments.map((shipment) => (
                  <article className={styles.shipment} key={shipment.id}>
                    <div className={styles.shipmentHeading}>
                      ...
                      <div className="row-actions">
                        <span
                          className={`status-badge ${shipment.status === "COMPLETED" ? "success" : shipment.status === "REVIEW" ? "inactive" : "active"}`}
                        >
                          {shipment.status}
                        </span>
                        <button
                          className="button button-primary button-compact"
                          disabled={shipment.status === "REVIEW"}
                          onClick={() => startShipment(shipment)}
                        >
                          운송장 열기
                        </button>
                      </div>
      ...
      {focus && activeShipment ? (
        <div className={styles.focusBackdrop}>
          ...
          >
            ...
            <h2>
              {focus.item.artist} · {focus.item.nameVer}
            </h2>
            <div className={styles.focusCount}>
              {focus.item.pickedQty}/{focus.item.requiredQty}
            </div>
            <div
              className={`${styles.progressBar} ${focus.kind === "COMPLETE" ? styles.completedBar : ""}`}
            >
              <span
                style={{
                  width: `${(focus.item.pickedQty / focus.item.requiredQty) * 100}%`,
                }}
              />
            </div>
            <div className={styles.focusMeta}>
              {focus.item.locations.map((location) => (
                <span className="status-badge" key={location.locationCode}>
                  {location.locationCode}
                </span>
              ))}
            </div>
            {focus.kind === "EXCESS" ? (
              <h3>이미 필요한 수량을 모두 피킹했습니다.</h3>
            ) : null}
            {activeShipment.manualQuantityAllowed &&
            focus.kind === "PICKING" ? (
              <div className={styles.manualBox}>
                <input
                  type="number"
                  min={1}
                  max={focus.item.requiredQty - focus.item.pickedQty}
                  value={manualQty}
...
export default function OutboundProgressPage() {
  return (
    <PermissionGuard permission="scan_inventory">
      <OutboundProgressContent />
    </PermissionGuard>
  );
}
