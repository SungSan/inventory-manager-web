...
function SessionContent() {
  ...
  async function openLocation(locationId: string, status: string) {
    setBusyLocationId(locationId); setError("");
  ...
  return (
    <div className={`page-stack ${styles.page}`}>
      ...
      <section className="panel">
        ...
        <div className={styles.locationCards}>{visibleLocations.map((row) => <article key={row.locationId} className={styles.locationCard}><div><span className={`status-badge ${row.status === "IN_PROGRESS" ? "active" : row.status === "COMPLETED" ? "success" : row.status === "CANCELLED" ? "inactive" : ""}`}>{row.status}</span><h4>{row.locationCode}</h4><p>{row.zone || "구역 없음"}</p></div><div className={styles.locationMetrics}><span><small>전산 SKU</small><strong>{row.systemSkuCount.toLocaleString()}</strong></span><span><small>확인 SKU</small><strong>{row.countedSkuCount.toLocaleString()}</strong></span><span><small>차이</small><strong>{row.differenceSkuCount} SKU / {row.differenceQty.toLocaleString()}개</strong></span></div>{row.status !== "CANCELLED" ? <button className={row.status === "COMPLETED" ? "button button-secondary" : "button button-primary"} onClick={() => void openLocation(row.locationId, row.status)} disabled={busyLocationId === row.locationId}>{busyLocationId === row.locationId ? "여는 중..." : row.status === "PENDING" ? "실사 시작" : row.status === "IN_PROGRESS" ? "실사 계속" : "결과 보기"}</button> : null}</article>)}{visibleLocations.length === 0 ? <p className="empty-state">조건에 맞는 로케이션이 없습니다.</p> : null}</div>
      </section>
    </div>
  );
}
